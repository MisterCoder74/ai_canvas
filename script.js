...
async function loadConfig() {
    try {
        const response = await fetch('user_data.php?type=config', { cache: 'no-store' });
        const data = await response.json();
        if (data.success) {
            appConfig = data.data;
        }
...
async function loadCanvasState() {
    try {
        const response = await fetch('user_data.php?type=canvas', { cache: 'no-store' });
        const data = await response.json();
        if (data.success) {
...
async function loadHistory() {
    try {
        const response = await fetch('user_data.php?type=history', { cache: 'no-store' });
        const data = await response.json();
        if (data.success) {
...
async function sendChatMessage() {
    ...
    try {
        ...
        showModalMessage('chatbotModal', 'error', 'Connection error. Check chatbot_api.php');
    }
...
async function generateImage() {
    ...
    try {
        const response = await fetch('image_gen_api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                apiKey: appConfig.apiKey,
        ...
        if (data.success) {
            ...
            // Display image using get_image.php for security
            const imageDisplayUrl = 'get_image.php?path=' + encodeURIComponent(data.imageUrl);
        ...
        console.error('API error:', error);
        showModalMessage('imageGenModal', 'error', 'Connection error. Check image_gen_api.php');
    }
...
async function analyzeImage() {
    ...
    try {
        const response = await fetch('image_analyze_api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                apiKey: appConfig.apiKey,
        ...
        console.error('API error:', error);
        showModalMessage('imageAnalyzerModal', 'error', 'Connection error. Check image_analyze_api.php');
    }
...
async function analyzeText() {
    ...
    try {
        const response = await fetch('text_analyze_api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                apiKey: appConfig.apiKey,
        ...
        console.error('API error:', error);
        showModalMessage('textAnalyzerModal', 'error', 'Connection error. Check text_analyze_api.php');
    }
...
function downloadGeneratedImage() {
    ...
    // Download via get_image.php
    const downloadUrl = 'get_image.php?path=' + encodeURIComponent(currentGeneratedImage);
    const a = document.createElement('a');
    a.href = downloadUrl;
...
function createMagnetElement(magnetData) {
    ...
    const imageDisplayUrl = 'get_image.php?path=' + encodeURIComponent(magnetData.imageUrl);
